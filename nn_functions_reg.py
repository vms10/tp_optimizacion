...
# Coeficientes de regularización (puedes ajustar o poner como argumento)
l2_reg = 1e-4
l1_reg = 0.0    # Pon >0 si quieres L1

def loss(params, coord, target):
    preds = batched_predict(params, coord)
    mse_loss = jnp.mean(jnp.square(preds - target))

    # === REGULARIZACION ===
    unpacked = unpack_params(params)
    l2 = sum([jnp.sum(jnp.square(w)) + jnp.sum(jnp.square(b)) for w, b in unpacked])
    l1 = sum([jnp.sum(jnp.abs(w)) + jnp.sum(jnp.abs(b)) for w, b in unpacked])
    reg_term = l2_reg * l2 + l1_reg * l1
